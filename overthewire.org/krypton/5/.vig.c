...
struct key_info* find_period(char* string){
	...
	if (indexes[max_sequence - 1] % indexes[max_sequence - 2] == 0){ 
		period_length = indexes[max_sequence - 2];

	} else {
		int index_in_loop, input, selection[3];
		printf("Statistics detected unusual properties; please choose a period length:\n");
		while(1){

			for(i = 0; i < 3; i++){
					index_in_loop = indexes[max_sequence - 3 + i];
